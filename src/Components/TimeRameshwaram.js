 ...
 const handleClick = () => {
  ...
  return (
    ...
    <Row className='mt-5'>
     ...
     <Col lg={6}>
     <Card style={{ width: '33rem' }} className='class-modal'>
    <div onClick={handleClick} className={`${(style === true) ? 'class-modal' : "hide-modal"}`}>
    
    
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'1rem'}}>
        <Modal.Title className='text-capitalize ms-1 fw-bold'>More about best time to travel to <br /> chennai</Modal.Title>
    ...
    <   hr />
      ...
      <Button style={{backgroundColor:'grey'}} variant="primary">close</Button>
      {/* <Button variant="primary"></Button> */}
      <Button variant="primary" onClick={handleShow} className="me-2">
        Save Changes
      </Button>
      <TimeRameshwaram key={1} placement={'end'} name={'end'} />
      </div>
      </div>
      <div className='canvas-modal'>
       <Offcanvas show={show} onHide={handleClose}>
        ...
        <p>  Winters, the best time to vist
...
export default TimeRameshwaram